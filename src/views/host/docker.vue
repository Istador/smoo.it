<template>
  ...
  >
    ...
    <p>
      ...
      But you can also run them on your system on which you already <a href="https://docs.docker.com/get-docker/" class="extern" target="_blank">installed docker</a>.
    </p>
    <p>
    ...
    <p>
      ...
      <a href="https://docs.docker.com/compose/" class="extern" target="_blank">docker-compose</a>
...
<b>services</b>:
  <b>server</b>:
    ...
    <p>
      ...
      <a href="https://github.com/Sanae6/SmoOnlineServer/blob/master/Dockerfile" class="extern" target="_blank">Dockerfile</a>.
    ...
    <p>
      To attach to the server on the command line interface, to send <router-link :to="{ name: 'host', params: { id: 'commands' } }">commands</router-link> to it, use <kbd>docker attach `docker-compose ps -q` --sig-proxy=false</kbd>.
      If you only want to look at the server logs without giving it any commands you can use <kbd>docker-compose logs --tail=20 --follow</kbd>.
    </p>
    <p>
      For additional security on Linux it's advised to define which user should run the server (default: <code>root</code>).
      Comment in the <code>user</code> line in the <code>docker-compose.yml</code> and provide the user ID and group ID to use (execute <kbd>echo $(id -u):$(id -g)</kbd> to get your own IDs).
